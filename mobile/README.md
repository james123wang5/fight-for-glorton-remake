...
这个版本从云端运行，Mac 合盖、关机或不在身边都不影响。因为游戏包含从原版导出的
素材，站点目前设为仅所有者可访问，没有公开分发这些素材。第一次在 iPhone 上使用：

1. 用 **Safari** 打开上面的地址。
2. 如果出现登录页，用部署此站点的同一个 ChatGPT/OpenAI 账号登录。
...
云端为紧凑的 1× 素材包，但保留六个角色、四张地图、全部动画和原有战斗逻辑；
