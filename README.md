...
> 当前仍是开发版本，并非 100% 完成。电脑版房间码联机已接入，部分动作衔接、打击反馈、
...
- 电脑联机：6 位房间码、双方同步选人、40Hz 权威服务器、输入缓冲、快照校正、断线重连及 1–22 级 AI。

联机启动、大厅按键、新电脑安装和公网部署见 [docs/ONLINE_MULTIPLAYER_CN.md](docs/ONLINE_MULTIPLAYER_CN.md)。
...
下载 GitHub Release 中的精简 1× 运行素材并启动游戏：
...
安装器会自动识别当前 GitHub 仓库、下载 ZIP、校验 SHA-256，并安全解压到
`assets/`。它只依赖 Python 标准库。需要 HIGH/Retina 高分辨率开发素材时，再按下文
从本地合法取得的 SWF 重新生成完整素材。

macOS、Windows、离线迁移和训练环境的完整步骤见
[`docs/INSTALL_AND_MIGRATE_CN.md`](docs/INSTALL_AND_MIGRATE_CN.md)。
...
Git 历史**不直接提交**约 1.4 GB 的完整 SWF 导出图片、声音、ActionScript 导出物和
中间清单。正式 Release 可附带一个约 35 MB 的依赖裁剪 1× 运行包，供已获授权的开发、
测试和跨电脑迁移使用；高分辨率源导出仍需使用者自行合法取得原版 SWF 后在本地生成。
原游戏素材的权利仍归原作者及相关权利人所有，公开再分发前应确认相应授权。
...
当前已冻结的 v5 正式模型保存在
`training/checkpoints/peach_purpose_v5/champion_level21_model.zip` 与
`champion_level22_model.zip`。其中 22 级是 2026-07-16 经人工实战认可的第 1 轮候选；
试玩时运行 `.venv-train/bin/python -m training.play_v5`。普通游戏、训练规则和模型文件
继续相互隔离，不会在启动 `python play.py` 时自动训练或改写模型。
...
2. 将 v5 的通用目的层扩展到其余角色与四张地图，保持逐角色/逐地图冠军门槛。
3. 加入房间模式、双端输入同步、预测与回滚联网对战。

详细执行顺序见
[`docs/AI_AND_ONLINE_ROADMAP_CN.md`](docs/AI_AND_ONLINE_ROADMAP_CN.md)。
