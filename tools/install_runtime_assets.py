...
MANIFEST = ROOT / "assets" / "manifests" / "glorton_manifest.json"
...
def request_bytes(url: str, *, accept: str = "application/octet-stream") -> bytes:
    request = Request(
        ...
        headers={
            "Accept": accept,
            "User-Agent": "glorton-runtime-assets-installer/1.1",
        },
    )
    try:
...
def _download_latest(repository: str, destination: Path) -> tuple[Path, str]:
...
def ensure_runtime_assets(
    ...
    *,
    ...
    """Install assets only when a fresh clone doesn't have the manifest."""

    if MANIFEST.is_file():
        return False
    local = archive.expanduser().resolve() if archive is not None else None
    with tempfile.TemporaryDirectory(prefix="glorton-download-") as temp_name:
        if local is not None:
            ...
            selected, expected = _download_latest(
                repository
...
def main() -> None:
    ...
    if MANIFEST.is_file() and args.archive is None:
        print(f"Runtime assets already present: {ROOT / 'assets'}")
        return
    if args.archive:
        archive = args.archive.expanduser().resolve()
        if not archive.is_file():
            raise SystemExit(f"Archive not found: {archive}")
        if args.sha256 and sha256(archive) != args.sha256.lower():
        ...
        with tempfile.TemporaryDirectory(prefix="glorton-download-") as temp_name:
            archive, expected = _download_latest(repository, Path(temp_name))
